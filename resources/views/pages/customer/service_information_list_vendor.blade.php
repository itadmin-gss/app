<div id="services_list_{{$data['service_id']}}" class="box span12 noMarginTop listSlide">
    <div class="box-header ">
        ...
        <div class="box-icon">
            <a href="javascript:;" class="btn-minimize"><i class="halflings-icon chevron-up"></i></a>
        </div>
    </div>
  ...
  @if(isset($data['due_date_'.$data['service_id']]))
        ...
        <div class="row-fluid browse-sec">
            ...
            <ul class="media-list ">
                ...
                <li id="service_image_{{$data['service_id']}}_{{$loop}}_li" class="hasicon"><img src="{{Config::get('app.request_images').'/'.$image}}" id="service_image_{{$data['service_id']}}_{{$loop}}_image"><img src='{{Config::get('app.img_dir')}}/close-button.png' onclick="removeservice('service_image_{{$data['service_id']}}_{{$loop}}')" class='icon'></li>

                {{Form::hidden('service_image_list_'.$data['service_id'].'[]', $image, array('id'=>'service_image_'.$data['service_id'].'_'.$loop.'_input'))}}
