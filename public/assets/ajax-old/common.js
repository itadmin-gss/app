...
// function ShowAdditionalServiceForm(){
    ...
    // }else 
    if(title.length == 0 || rate <= 0 || quantity <= 0 ){
        $("#additional_flash_dan").slideDown();
        setInterval(function() {
           $("#additional_flash_dan").slideUp();
       }, 5000); 
...
function updateRequestedService(id){
    ...
    var notes_for_vendors= $('#notes_for_vendors').val();
       ...
       $.ajax({
        ...
        data:{vendors_price:vendors_price,customer_price:customer_price,quantity:quantity,
  ...
  function updateAdditionalitem(id){
    ...
    if( rate <= 0 || quantity <= 0){
       ...
       $.ajax({
        ...
        data:{description:description,quantity:quantity,rate:rate,customer_price:customer_price},
        success: function(data) {
                //console.log(data);
                location.reload();
                $('#overlay').remove();
...
{
	// var over = '<div id="overlay" class="cstmOverlay">'+'<img id="loading" src="'+baseurl+'public/assets/img/loader.gif">'+'</div>';
 //    $(over).appendTo('#before_view_image_'+order_id);  
    $.ajax({
        type: 'POST',   
        ...
        success: function(data) {
            if (data) 
            {
                if (type == 'before')
                {
                    ...
                    //  $('.img-thumbnail').on('load', function(){
                    //     $('#overlay').css('display', 'none');
                    //     //$('#before_view_modal_image_' + order_detail_id).css("overflow", "auto");
...
function saveAdditionalVendorNote(additional_id)
{
    ...
    if (additional_vendors_notes == '' || additional_vendors_notes == "undefined")
    {
    ...
    else
    {
        ...
        $.ajax({
            ...
            success: function(data) {
                 //alert(data);
                $('#textarea-vendor-note-'+additional_id).attr("style", "display:none;");;
                window.location.reload();
}
});
    }
...
function editAdditionalVendorNoteButton(additional_item_id)
{
    ...
    $('#save-additional-vendor-notes-button-' + additional_item_id).attr("style","display:block;"); 
}
